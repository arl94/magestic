...
for i, r in low_reads.iterrows():
	...
	candidates = high_reads[(high_reads['mut_id'] == rsseqid) & (high_reads['subpool'] == r['subpool'])]
	...
	"""		else:
							clusters[rqseqid] = rqseqid
					else:
						clusters[rqseqid] = rqseqid"""

new_df = pd.DataFrame(nl, columns=['qseqid1', 'qseqid2', 'barcode_ed', 'guide_ed', 'donor_ed', 'read_count_1', 'read_count_2'])
new_df.to_csv("new_df_"+ FILENAME + "_" + str(segment) + ".csv")

pickle_out = open(FILENAME.split(".csv")[0] + "_" + str(segment)  + ".cluster_dict", "wb")
