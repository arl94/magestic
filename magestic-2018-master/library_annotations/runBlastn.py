...
parser.add_argument('-flength', required=False, default=140, help="length of forward read to use in mapping results (default=140)", action='store', dest='forward_length')
parser.add_argument('-rlength', required=False, default=None, help="length of reverse read to use in mapping results (default=None)", action='store', dest='reverse_length')
...
print('./blastn -query ' + args.filename + ' -db ' + args.database + ' -outfmt "10 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qseq sseq" -out ' + OUTPUT_HEADER + '_blast.csv -num_alignments 1')
os.system('./blastn -query ' + args.filename + ' -db ' + args.database + ' -outfmt "10 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qseq sseq" -out ' + OUTPUT_HEADER + '_blast.csv -num_alignments 1') # Run blastn
df = pd.read_csv(OUTPUT_HEADER + '_blast.csv', names=['qseqid', 'sseqid', 'pident', 'length', 'mismatch', 'gapopen', 'qstart', 'qend', 'sstart', 'send', 'evalue', 'bitscore', 'qseq', 'sseq'])
...
if flength != "None": flength = int(flength)
else:
	flength = eval(flength)
...
df.to_csv(OUTPUT_HEADER + "_blast_mapped.csv", index=False)